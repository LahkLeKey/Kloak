...
test.describe('Manage app status', () => {
  test('pending app shows Activate button', async ({ page }) => {
    ...
    const appCard = page.getByTestId('app-card').filter({ hasText: 'Lifecycle App' });
  ...
  test('activating a pending app transitions it to active', async ({ page }) => {
    ...
    const appCard = page.getByTestId('app-card').filter({ hasText: 'App to Activate' });
  ...
  test('suspending an active app transitions it to suspended', async ({ page }) => {
    const dep = await createDeployment('suspend-test', 'acme');
    const app = await registerApp(dep.id, 'App to Suspend', 'suspend.acme.com');
    ...
    const appCard = page.getByTestId('app-card').filter({ hasText: 'App to Suspend' });
    await appCard.getByTestId('suspend-btn').click();

    await expect(appCard.getByTestId('status-badge')).toHaveText('Suspended');
  });

  test('revoking an active app removes activate and suspend buttons', async ({ page }) => {
    const dep = await createDeployment('revoke-test', 'acme');
    const app = await registerApp(dep.id, 'App to Revoke', 'revoke.acme.com');
    ...
    const appCard = page.getByTestId('app-card').filter({ hasText: 'App to Revoke' });
