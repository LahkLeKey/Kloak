...
test.describe('Register external application', () => {
  test('operator can register a new app via the form', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: /Register external application/i })).toBeVisible();

    // Fill in the form
    await page.getByTestId('name-input').fill('Company Portal');
    await page.getByTestId('description-input').fill('Internal employee portal with SSO');
    await page.getByTestId('origins-input').fill('https://portal.acme.com');
    await page.getByTestId('redirects-input').fill('https://portal.acme.com/auth/callback');
  ...
  test('registered app shows pending status initially', async ({ page }) => {
    ...
    await page.getByTestId('redirects-input').fill('https://app.example.com/callback');
    await page.getByTestId('submit-btn').click();

    await expect(page).toHaveURL(`/deployments/${dep.id}`);

    const appCard = page.getByTestId('app-card').filter({ hasText: 'Pending App' });
    await expect(appCard.getByTestId('status-badge')).toHaveText('Pending');
  });

  test('form validation requires name, origins, and redirects', async ({ page }) => {
  ...
  test('operator can navigate back to deployment without submitting', async ({ page }) => {
