...
export class InMemoryKeycloakClient implements KeycloakClient {
  ...
  async readLiveState(deployment: Deployment): Promise<KeycloakLiveState> {
    return (
      this.liveStateByDeploymentId.get(deployment.id) ?? {
...
export class InMemoryInfrastructureStateReader implements InfrastructureStateReader {
  ...
  async readLiveState(target: ProvisioningTarget): Promise<InfrastructureLiveState> {
    return (
      this.liveStateByTarget.get(targetKey(target)) ?? {
        target,
        ingressHosts: [],
        dnsRecords: [],
        secrets: [],
...
function targetKey(target: ProvisioningTarget): string {
  return `${target.cloudProvider}:${target.accountId}:${target.projectId}:${target.environment}`;
}
