...
import type {CoreApi, CreateAuthFlowInput, CreateDeploymentInput, CreateDeploymentVersionInput, RegisterExternalAppInput,} from '../core/index.ts';
import type {AuthFlowId, DeploymentId, DeploymentStatus, ExternalAppId, ExternalAppStatus, LinkAccountInput, LoginInput, OAuthAuthorizeInput, OAuthCallbackInput, SignupInput, UserId,} from '../shared/index.ts';
...
export class AdminHandler {
  ...
  async handle(req: HttpRequest): Promise<HttpResponse> {
    ...
    if (method === 'GET' && path === '/deployments/:id') {
      const deployment =
          await this.core.getDeployment(params.id as DeploymentId);
      if (deployment === null) {
    ...
    if (method === 'PUT' && path === '/deployments/:id/status') {
      ...
      const deployment = await this.core.markDeploymentStatus(
          params.id as DeploymentId, newStatus);
      return {status: 200, body: deployment};
    }
    ...
    if (method === 'PUT' && path === '/apps/:appId/status') {
      ...
      const app = await this.core.updateExternalAppStatus(
          params.appId as ExternalAppId, newStatus);
      return {status: 200, body: app};
    }
    ...
    if (method === 'DELETE' &&
        path === '/users/:userId/link-account/:provider') {
      const user = await this.core.unlinkAccount(
          params.userId as UserId, params.provider);
      return {status: 200, body: user};
    }
...
export function matchRoute(
    reqPath: string, pattern: string): Record<string, string>|null {
...
export async function nodeHttpAdapter(
    handler: AdminHandler, req: IncomingMessage,
  ...
  const response = matched !== null ? await handler.handle(matched) :
